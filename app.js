document.addEventListener('DOMContentLoaded', () => {
  ...
  // --- CONFIGURATION ---
    ...
    // --- RESPONSIVE RENDER SHELL ---
    function renderAppShell() {
        appScreen.innerHTML = `
            <div class="flex h-screen bg-gray-100 overflow-hidden">
                <!-- Mobile Overlay -->
                <div id="mobile-overlay" onclick="toggleSidebar()" class="fixed inset-0 bg-black opacity-50 z-20 hidden md:hidden"></div>

                <!-- Sidebar -->
                <aside id="sidebar" class="fixed inset-y-0 left-0 z-30 w-64 bg-gray-800 text-white transform -translate-x-full transition-transform duration-300 md:relative md:translate-x-0 flex flex-col">
                    <div class="h-16 flex items-center justify-between px-4 border-b border-gray-700">
                        <span class="text-2xl font-bold">EduMate</span>
                        <button onclick="toggleSidebar()" class="md:hidden text-gray-400 hover:text-white"><i data-lucide="x"></i></button>
                    </div>
                    <nav class="flex-1 p-2 space-y-2 overflow-y-auto" id="sidebar-nav"></nav>
                    <div class="p-4 border-t border-gray-700">
                        <button onclick="location.reload()" class="w-full flex items-center px-4 py-2 hover:bg-red-600 rounded transition-colors"><i data-lucide="log-out" class="w-5 h-5 mr-3"></i> Logout</button>
                    </div>
                </aside>

                <!-- Content -->
                <div class="flex-1 flex flex-col min-w-0 overflow-hidden">
                    <header class="bg-white shadow-md px-4 h-16 flex items-center justify-between z-10">
                        <div class="flex items-center">
                            <button onclick="toggleSidebar()" class="md:hidden mr-4 text-gray-600"><i data-lucide="menu" class="w-6 h-6"></i></button>
                            <h1 class="text-xl font-semibold truncate">${currentView}</h1>
                        </div>
                        <span class="font-medium text-sm md:text-base truncate ml-2">Hi, ${teacherName.split(' ')[0]}!</span>
                    </header>
                    <main id="main-content" class="flex-1 overflow-x-hidden overflow-y-auto p-4 md:p-6"></main>
                </div>
            </div>`;
    ...
    function renderSidebarLinks() {
        ...
        document.getElementById('sidebar-nav').innerHTML = links.map(l => `
            <a href="#" class="sidebar-link flex items-center px-4 py-3 text-sm font-medium rounded-lg transition-colors hover:bg-gray-700 ${currentView===l.name?'bg-indigo-600 text-white':''}" data-page="${l.name}">
               <i data-lucide="${l.icon}" class="w-5 h-5 mr-3"></i> ${l.name}
            </a>`).join('');
            
        document.querySelectorAll('.sidebar-link').forEach(l => l.addEventListener('click', (e) => {
            e.preventDefault(); renderPage(e.currentTarget.dataset.page);
    ...
    function renderPage(p) {
        currentView = p; renderSidebarLinks();
    ...
    function renderDashboard(c) {
        ...
        c.innerHTML = `
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div class="bg-white p-6 rounded shadow flex items-center"><i data-lucide="users" class="w-8 h-8 text-indigo-600 mr-4"></i><div><p class="text-gray-500">Students</p><h2 class="text-2xl font-bold">${classesData.reduce((acc,cl)=>acc+cl.students.length,0)}</h2></div></div>
                <div class="bg-white p-6 rounded shadow flex items-center"><i data-lucide="book-open" class="w-8 h-8 text-green-600 mr-4"></i><div><p class="text-gray-500">Classes</p><h2 class="text-2xl font-bold">${classesData.length}</h2></div></div>
                <div class="bg-white p-6 rounded shadow col-span-1 md:col-span-3"><h2 class="text-xl font-bold mb-4">Today's Schedule (${today})</h2>${tc.length ? tc.map(t=>`<div class="p-3 bg-gray-50 rounded mb-2 font-semibold flex justify-between"><span>${t.subject}</span><span class="text-gray-500 text-sm">${t.time}</span></div>`).join('') : '<p class="text-gray-500">No classes.</p>'}</div>
            </div>`;
    }
    ...
    function renderClassLists(c) {
        c.innerHTML = `
            <div class="flex flex-col md:flex-row justify-between mb-6 gap-4"><h2 class="text-2xl font-bold">Classes</h2><button onclick="addClass()" class="px-4 py-2 bg-green-600 text-white rounded w-full md:w-auto">Add Class</button></div>
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">${classesData.map(cls => `<div class="bg-white p-6 rounded shadow relative"><button class="absolute top-4 right-4 text-red-500" onclick="delClass('${cls.id}')"><i data-lucide="trash-2"></i></button><h3 class="text-xl font-bold text-indigo-700 truncate pr-8">${cls.name}</h3><p class="text-sm text-gray-600 mb-4">${cls.coordinatorName}</p><button class="w-full bg-indigo-600 text-white py-2 rounded" onclick="viewClass('${cls.id}')">View Students</button></div>`).join('')}</div>`;
        window.addClass = () => showModal(`<h2 class="text-xl font-bold mb-4">Add Class</h2><input id="n-cn" placeholder="Name" class="w-full border p-2 mb-2 rounded"><input id="n-cc" placeholder="Coordinator" class="w-full border p-2 mb-2 rounded"><input id="n-cp" placeholder="Phone" class="w-full border p-2 mb-4 rounded"><div class="flex justify-end gap-2"><button onclick="closeModal()" class="bg-gray-200 px-4 py-2 rounded">Cancel</button><button onclick="saveClass()" class="bg-indigo-600 text-white px-4 py-2 rounded">Save</button></div>`);
        ...
        window.openProfile = (sid) => {
            ...
            showModal(`
                ...
                <div class="space-y-3 mb-4">
                    ...
                    <div class="flex justify-between items-center border-b pb-2"><span>Phone:</span> <div>${s.phone} ${s.phone?`<a href="tel:${s.phone}" class="bg-green-100 text-green-700 px-2 rounded ml-2">Call</a>`:''}</div></div>
                    <div class="flex justify-between items-center border-b pb-2"><span>Parent:</span> <div>${s.parentPhone} ${s.parentPhone?`<a href="tel:${s.parentPhone}" class="bg-green-100 text-green-700 px-2 rounded ml-2">Call</a>`:''}</div></div>
                ...
                <div class="flex justify-end gap-2"><button onclick="closeModal()" class="bg-gray-200 px-4 py-2 rounded">Close</button><button onclick="updateProfile('${s.id}')" class="bg-indigo-600 text-white px-4 py-2 rounded">Save</button></div>
            `);
        };

        window.updateProfile = async (sid) => {
            await apiCall(`/api/students/${sid}`, 'PUT', { address: val('p-addr'), previousMarks: val('p-marks') });
            alert('Saved'); closeModal(); await loadAllData();
        };

        window.addStudent = (cid) => showModal(`<h2 class="text-xl font-bold mb-4">Add Student</h2><input id="n-sn" placeholder="Name" class="w-full border p-2 mb-2 rounded"><input id="n-sr" placeholder="Roll" class="w-full border p-2 mb-2 rounded"><input id="n-se" placeholder="Email" class="w-full border p-2 mb-2 rounded"><select id="n-ss" class="w-full border p-2 mb-2 rounded"><option>Day Scholar</option><option>Hosteller</option></select><input id="n-sp" placeholder="Student Phone" class="w-full border p-2 mb-2 rounded"><input id="n-pp" placeholder="Parent Phone" class="w-full border p-2 mb-4 rounded"><div class="flex justify-end gap-2"><button onclick="closeModal()" class="bg-gray-200 px-4 py-2 rounded">Cancel</button><button onclick="saveStud('${cid}')" class="bg-indigo-600 text-white px-4 py-2 rounded">Save</button></div>`);
        ...
        window.saveStud = async (cid) => { 
            ...
            if(res.error) { alert(res.error); } else { closeModal(); await loadAllData(); viewClass(cid); }
        };
    ...
    function renderTimetable(c) {
        c.innerHTML = `<h2 class="text-2xl font-bold mb-6">Timetable</h2><div class="space-y-4 md:space-y-6">${['Monday','Tuesday','Wednesday','Thursday','Friday','Saturday'].map(d=>`<div class="bg-white p-4 rounded shadow"><h3 class="font-bold mb-2">${d}</h3>${timetableData.filter(t=>t.day===d).sort((a,b)=>a.time.localeCompare(b.time)).map(t=>`<div class="flex justify-between p-2 bg-gray-50 mb-1 rounded"><span><b>${t.time}</b> ${t.subject}</span><button onclick="delTime('${t.id}')" class="text-red-500">x</button></div>`).join('')}<button onclick="addTime('${d}')" class="text-indigo-600 text-sm mt-2">+ Add</button></div>`).join('')}</div>`;
        window.addTime=(d)=>showModal(`<h2 class="text-xl font-bold mb-4">Add to ${d}</h2><input type="hidden" id="t-d" value="${d}"><input type="time" id="t-t" class="w-full border p-2 mb-2 rounded"><input id="t-s" placeholder="Subject" class="w-full border p-2 mb-2 rounded"><input id="t-l" placeholder="Location" class="w-full border p-2 mb-4 rounded"><div class="flex justify-end gap-2"><button onclick="closeModal()" class="bg-gray-200 px-4 py-2 rounded">Cancel</button><button onclick="saveTime()" class="bg-indigo-600 text-white px-4 py-2 rounded">Save</button></div>`);
    ...
    function renderExamsPage(c) {
        ...
        window.mkEx=()=>showModal(`<h2 class="text-xl font-bold mb-4">New Exam</h2><select id="ne-c" class="w-full border p-2 mb-2 rounded"><option value="">Class</option>${classesData.map(c=>`<option value="${c.id}">${c.name}</option>`)}</select><input id="ne-t" placeholder="Title" class="w-full border p-2 mb-2 rounded"><input id="ne-m" placeholder="Max" type="number" class="w-full border p-2 mb-4 rounded"><div class="flex justify-end gap-2"><button onclick="closeModal()" class="bg-gray-200 px-4 py-2 rounded">Cancel</button><button onclick="svEx()" class="bg-indigo-600 text-white px-4 py-2 rounded">Save</button></div>`);
    ...
    function renderAssignmentsPage(c) {
        c.innerHTML = `
            <div class="bg-white p-6 rounded shadow max-w-2xl mx-auto">
                ...
                <div class="mb-4"><label class="block font-bold mb-1">Attach File</label><input type="file" id="asg-f" class="w-full border p-2 rounded"></div>
        ...
        window.sendAsg = async () => {
            ...
            if(fileInput.files.length > 0) {
                ...
                } catch(e) { btn.disabled = false; btn.textContent = "Send via Email"; return alert("Server error"); }
            }

            if(!confirm(`Send to ${list.length} students?`)) { btn.disabled = false; btn.textContent = "Send via Email"; return; }
    ...
    function renderReportsPage(c) {
        ...
        window.loadRepData = async () => {
            ...
            document.getElementById('rep-res').innerHTML = `
                ...
                <div class="bg-white p-6 rounded shadow"><h3 class="font-bold text-xl mb-4">Exam Performance</h3><table class="w-full text-left"><thead><tr class="border-b"><th class="pb-2">Exam</th><th class="pb-2">Obtained</th><th class="pb-2">Total</th></tr></thead><tbody>${exams.map(e=>`<tr><td class="py-2 border-b">${e.title}</td><td class="py-2 border-b font-bold">${e.obtained}</td><td class="py-2 border-b text-gray-500">${e.total}</td></tr>`).join('')}</tbody></table></div>`;
        };
    }
